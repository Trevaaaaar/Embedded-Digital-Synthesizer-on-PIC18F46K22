...
| 19         | Note profile struct implementation               | Verify structured sound parameter storage                | Stored volume, waveform, attack, decay, sustain, release, tremolo depth, and pitch in a note_profile struct | Confirmed that all sound settings could be grouped into one reusable sound profile |
| 20      	 | EEPROM read/write bring-up	                      | Verify access to internal PIC EEPROM	                   | Wrote test bytes to EEPROM addresses and read them back after reset	                  | Confirmed EEPROM could store non-volatile data
21	Save note profile to EEPROM	Verify that a complete synth sound profile could be stored	Copied the active note_profile fields into EEPROM memory	Confirmed volume, waveform, envelope, tremolo, and pitch values could be saved
22	Load note profile from EEPROM	Verify that saved settings could be restored into the synth	Read EEPROM values back into the active note profile and applied them to the audio engine	Confirmed saved presets could be recalled and used for sound generation
23	EEPROM power-cycle persistence test	Verify non-volatile preset storage	Saved a preset, disconnected/restarted the PIC, then loaded the preset again	Confirmed user-defined synth settings persisted across power cycles
24	EEPROM preset integration with UI/audio parameters	Verify preset loading did not break real-time audio behavior	Loaded EEPROM values into UI/audio parameter structs, then committed them to the sound engine	Confirmed preset changes could be applied cleanly without disrupting the audio ISR
25	Multiple preset slot test	Verify storage of more than one sound profile	Saved different profiles to separate EEPROM address ranges and reloaded each slot	Confirmed multiple stored presets could be selected and restored independently
